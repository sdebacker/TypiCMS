...
class EloquentMenulink extends RepositoriesAbstract implements MenulinkInterface
{
    ...
    public function getMenu($name)
    {
        ...
        $models = $this->model->select(
            ...
            'menulinks.page_id',
            'menulinks.class',
            'menulink_translations.title',
            'menulink_translations.status',
            'menulink_translations.url',
            'pages.is_home',
            'page_translations.uri as page_uri',
    ...
    public function filter(Collection $models)
    {
        $models->each(function ($menulink) {
            ...
            if ($menulink->page_uri == $activeUri or
                    (
                        strlen($menulink->page_uri) > 3 and
                        preg_match('@^'.$menulink->page_uri.'@', $activeUri)
                    )
                ) {
